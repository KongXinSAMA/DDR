...
from Utils.BaseModel import U_Shadow_MF, SBR, AffineTransform, RealNVP
...
def train_eval(config):
    ...
    user_ivae_mean = torch.load(data_params["Shadow_path"] + "item_mean.pt", map_location='cpu').to(device)

    model = SBR(n_users, n_items, config["embedding_dim"], U_Shadow_size=user_ivae_mean.shape[1], init=data_params["init"], device=device).to(device)
    ...
    for epoch in range(config["epochs"]):
        ...
        for index, (uid, iid, rating) in enumerate(train_loader):
            ...
            loss = mf_loss_function(rating, preds)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    if args.tune:
        ...
        if args.data_params["name"] == 'coat':
            ...
            test_param = {'lr': 5e-04, 'weight_decay': 1e-05, 'embedding_dim': 256}
