...
item = pd.read_csv(r"C:\Code\Frontdoor\SDR\DataSet\coat\train.csv").to_numpy().astype(float)

train_df = pd.read_csv(r"C:\Code\Frontdoor\SDR\DataSet\coat\train.csv")
...
pop= sorted_counts[sorted_counts['count'] < 23]['count'].sum()
unpop =sorted_counts[sorted_counts['count'] >= 23]['count'].sum()
